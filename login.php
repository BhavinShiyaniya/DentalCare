...
if (isset($_POST["signin"])) {
    ...
    $check_email = mysqli_query($conn, "SELECT id FROM `users` WHERE email='$email' AND password='$password' AND status='1' AND type='$type'") or ('die query');

    $check_role = mysqli_query($conn, "SELECT * FROM `users` WHERE type='$type'") or ('die query');
    $row1 = mysqli_fetch_assoc($check_role);
    $_SESSION["type"] = $row1['type'];

    if (mysqli_num_rows($check_email) > 0 && $type=="admin") {
        $row = mysqli_fetch_assoc($check_email);
        $_SESSION["user_id"] = $row['id'];
        header("Location: admin.php");
        // header("Location: welcome.php");
    } elseif (mysqli_num_rows($check_email) > 0 && $type=="user") {
      $row = mysqli_fetch_assoc($check_email);
      $_SESSION["user_id"] = $row['id'];
      header("Location: index.php");
    } else {
      ...
      >
        <div class="flex flex-col overflow-y-auto md:flex-row">
          ...
          <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
            ...
            <button type="submit"
            class=" mt-4 w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple" name="signin">
            Log in
        </button>
      </form>
              <hr class="my-8" />
